// <auto-generated>
//   AI-Refactored: MissionEvaluator.cs (Supreme Arbitration/Overlay Edition – June 2025, Ultra-Realism Blueprint, Mastermoveplan Compliant)
//   Overlay/event-only mission intent, pooled, error-shielded, anti-teleport, no tick-move, zero direct transform, 1:1 multiplayer/headless parity.
//   All movement overlays/event handlers use arbitration, triple NavMesh/Y/dedup validation. No tick/cooldown gating, only one overlay at a time.
//   MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Missions.Subsystems
{
    ...
    /// <summary>
    /// Overlay-only mission intent evaluator: arbitration/event, pooled, squad/solo, bulletproof, anti-stuck, multiplayer/headless safe.
    /// No tick/cooldown/teleport. Only overlays may trigger moves. Everything pooled and zero-alloc.
    /// </summary>
    public sealed class MissionEvaluator
    {
        ...
        /// <summary>
        /// Initializes a new MissionEvaluator overlay for the given bot and cache.
        /// </summary>
        ...
        /// <summary>
        /// Returns overlay intent direction for movement overlays (squad/solo).
        /// Pooled, never allocates, always normalized.
        /// </summary>
        public Vector3 GetCurrentIntentDirection()
        {
            ...
            // Prefer squad flock intent if available.
            ...
            // Fallback: use look direction.
            return _bot.LookDirection.normalized;
        }

        #endregion

        #region Mission/Extraction Overlay Checks

        /// <summary>
        /// True if bot is close to 60%+ squadmates within cohesion range (overlay-only).
        /// Bulletproof, pooled, error-isolated.
        /// </summary>
        ...
        /// <summary>
        /// True if bot should extract early based on loot count and personality. Overlay only.
        /// Uses pooling for item queries.
        /// </summary>
        public bool ShouldExtractEarly()
        {
            try
            {
                ...
                finally { TempListPool.Return(items); }
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Overlay/event-based extraction: arbitration/event-only, pooled, anti-teleport, zero direct transform.
        /// </summary>
            ...
            try
            {
                    ...
                    NavMesh.SamplePosition(best.transform.position, out NavMeshHit hit, NavSampleRadius, AIRefactoredLayerMasks.NavMeshAll))
                {
                    ...
                    if (!BotMovementHelper.ShouldMove(_bot, drifted))
                        return; // Deduplication after drift.
        ...
        #region Overlay Arbitration Anti-Stuck

        /// <summary>
        /// Overlay arbitration anti-stuck/fallback: never direct transform, triple arbitration/validation, bulletproof.
        /// </summary>
        public void UpdateStuckCheck(float now)
        {
            try
            {
                ...
                bool cooldownPassed = now - _stuckSince > StuckCooldown;
                bool stuckLongEnough = now - _lastMoveTime > StuckDuration;
                bool notRecentlyFellback = now - _lastStuckFallbackTime > StuckCooldown;

                if (stuckLongEnough && cooldownPassed && notRecentlyFellback && _fallbackAttempts < 2)
                {
                    _stuckSince = now;
                    _lastStuckFallbackTime = now;
                    _fallbackAttempts++;
