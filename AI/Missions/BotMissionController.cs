// <auto-generated>
//   AI-Refactored: BotMissionController.cs (Ultimate Overlay/Event, Supreme Arbitration, June 2025)
//   Bulletproof: All mission/objective/squad/extraction logic is null-guarded, overlay/event-ticked, and squad-aligned.
//   All errors locally isolated. No per-module tick, no disables, overlay/event-driven only. MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Missions
{
    ...
    /// <summary>
    /// Drives bot missions, objectives, extraction, and squad overlays. Overlay/event only; BotBrain-ticked.
    /// Full arbitration/overlay parity: no disables, bulletproof, headless/SPT/FIKA/client safe, squad-aligned.
    /// </summary>
    public sealed class BotMissionController
    {
        ...
        private const float ExtractionIntentDistance = 5.5f;
        ...
        public BotMissionController(BotOwner bot, BotComponentCache cache)
        {
            try
            {
                ...
                var profile = cache.AIRefactoredBotOwner?.PersonalityProfile ?? BotPersonalityProfile.Default;
        ...
        #region Overlay/Event Tick Logic

        /// <summary>
        /// Overlay/event mission tick, called via BotBrain only.
        /// </summary>
        public void Tick(float time)
        {
            try
            {
                ...
                // Squad rejoin/hold logic (overlay only)
                ...
                // Overlay-driven loot sync: triggers squad share, voice, and squad-aware overlays
                if (!_inCombatPause &&
                ...
                // Overlay-driven extraction intent sync: triggers exit-located VO and overlays
                if (_missionType == MissionType.Extract &&
                    _evaluator.IsGroupAligned() &&
                    _extraction.HasValidExtractionPoint &&
                    Vector3.Distance(_bot.Position, _extraction.CurrentExtractionPosition) < ExtractionIntentDistance)
                {
                    _voice.OnExitLocated();
                    _objectives.OnObjectiveReached(_missionType);
                }
            }
            catch (Exception ex)
        ...
        #region Mission Control & Overlay Callbacks

        /// <summary>
        /// Forces a specific mission type, disabling auto-switching until cleared.
        /// </summary>
        public void SetForcedMission(MissionType mission)
        {
            try
            {
                ...
                _forcedMission = true;
                Logger.LogInfo($"[BotMissionController] Forced mission: {mission}");
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Overlay/event callback for objective reached. Handles overlays, squad, loot/pose, and voice.
        /// </summary>
        ...
        /// <summary>
        /// Overlay/event callback for escalation to Fight mission state.
        /// </summary>
        ...
        #region Overlay-Driven State/Utility

        private bool IsPanicking()
        {
            var panic = _cache.PanicHandler;
            return panic != null && panic.IsPanicking;
        }

        /// <summary>
        /// Overlay-driven anti-stuck check using public accessor from BotMovementController.
        /// </summary>
        private bool IsLocallyStuck()
        {
            // Use MovementController's public anti-stuck check for overlay arbitration.
            return _cache.Movement != null && _cache.Movement.IsInAntiStuckState();
        }

        private bool ShouldExtractEarly()
        {
            return _evaluator.ShouldExtractEarly();
        }
