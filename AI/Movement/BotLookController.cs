// <auto-generated>
//   AI-Refactored: BotLookController.cs (Beyond Diamond, Ultra-Realism Overlay Edition – June 2025)
//   Overlay/event-driven only, anti-spam, bulletproof stabilization, ultra-human-like idle/aim/look.
//   Never snaps, never jitters, never over-corrects. Multiplayer/headless-safe. MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Movement
{
    ...
    /// <summary>
    /// Overlay/event-driven, fully stabilized, humanized look/aim/scan logic.
    /// Anti-jitter, anti-snap, anti-robot. Squad/cover/idle aware. Bulletproof error handling.
    /// </summary>
    public sealed class BotLookController
    {
        ...
        /// <summary>
        /// Overlay/event-only update. Humanizes all look/aim transitions.
        /// </summary>
        public void Tick(float deltaTime)
        {
            try
            {
                ...
                // --- 1. Threat tracking: locks out idle scan if present ---
                    ...
                    if (EFTPlayerUtil.IsValid(enemy))
                    {
                        ...
                        if ((nextTarget - _lastLookTarget).sqrMagnitude > StabilityThresholdSqr)
                        {
                            if (Time.time - _lastLookSwitchTime > LookSwitchCooldown)
                            {
                                ...
                                // Add micro-overshoot for realism
                                if (!_overshootPending)
                                {
                                    _overshootPending = true;
                                    Vector3 dir = (nextTarget - origin).normalized;
                ...
                if (!switching && !hasThreat)
                {
                    ...
                    {
                        // Small micro-head jitter even when holding
                        if (Time.time > _nextHeadJitterTime)
                        {
                            float jitter = UnityEngine.Random.Range(-IdleHeadJitter, IdleHeadJitter);
                            _currentYaw += jitter * 0.11f;
                            _currentPitch += UnityEngine.Random.Range(-IdleHeadJitter * 0.22f, IdleHeadJitter * 0.13f);
                            _nextHeadJitterTime = Time.time + UnityEngine.Random.Range(0.7f, 1.5f);
                        }
                        nextTarget = _lastLookTarget;
                    }
                ...
                // --- 4. Humanized overshoot/follow-through ---
                if (_overshootPending)
                {
                    targetYaw = Mathf.Lerp(targetYaw, _overshootTargetYaw, 0.4f);
                    targetPitch = Mathf.Lerp(targetPitch, _overshootTargetPitch, 0.28f);
        ...
        /// <summary>
        /// Set an external fallback look target for overlays (idle/scan/squad).
        /// </summary>
        ...
        /// <summary>
        /// Validates final look target, clamps to LOS, blocks, and safe distance.
        /// </summary>
