// <auto-generated>
//   AI-Refactored: FlankPositionPlanner.cs (Ultra-Realism Overlay/Intent-Only, Squad/Personality/Role Safe, June 2025+)
//   Never issues movement. Overlay-only intent/flank/fallback suggestions. Bulletproof: zero alloc hot-path, anti-cluster, squad-role aware, error-shielded, SPT/FIKA/headless safe.
//   MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Movement
{
    ...
    /// Squad, personality, and role safe. Handles fallback, anti-cluster, error, and squad overlays.
    /// </summary>
    public static class FlankPositionPlanner
    {
        #region Constants

        private const float BaseOffset = 4.05f;
        ...
        private const float OffsetVariation = 0.55f;
        private const float VerticalTolerance = 1.95f;
        private const float MissChanceDefault = 0.053f;
        private const float SquadRepelRadius = 2.13f;
        private const float GroupOffsetJitter = 0.52f;
        private const float FallbackRadius = 7.8f;
        private const int MaxRandomFlankAttempts = 7;
        private const float FarClusterExtraRadius = 1.6f;
        private const float ClusterTooClose = 0.65f;
        private const float PersonalityMissBonus = 0.031f;
        private const float SquadStackPenalty = 0.46f;
        private const float CoverBiasBonus = 0.42f;
        ...
        /// Optionally accepts squad/personality for realism overlays.
        /// </summary>
            ...
            Vector3? coverNormal = null)
        {
            ...
            // Cover bias overlay: bias toward cover side if provided
            if (coverNormal.HasValue && UnityEngine.Random.value < CoverBiasBonus)
            {
                float sideDot = Vector3.Dot(Vector3.Cross(toEnemy, Vector3.up), coverNormal.Value.normalized);
                Side coverSide = sideDot > 0f ? Side.Right : Side.Left;
                if (TrySide(botPos, toEnemy, coverSide, out flankPoint, squad, personality))
                    return true;
            }

            // Try preferred, then fallback side
        ...
        /// <summary>
        /// Suggests a squad/personality/cover overlay-intent flank point using enemy forward and bot angle.
        /// </summary>
            ...
            Vector3? coverNormal = null)
        {
            ...
            return TryFindFlankPosition(botPos, enemyPos, out flankPoint, side, squad, personality, coverNormal);
        }
            ...
            BotPersonalityProfile personality = null)
        {
            ...
            float stackPenalty = 0f;

            if (personality != null)
            {
                // Cautious/low-aggression miss/jitter/stack penalty overlays
                missChance += PersonalityMissBonus * (1.0f - personality.AggressionLevel) + personality.Caution * PersonalityMissBonus;
                offsetVariance += 0.13f * personality.Caution;
                groupJitter += 0.12f * personality.Caution;
                stackPenalty += SquadStackPenalty * personality.Caution;
            }
            ...
            for (int i = 0; i < MaxAttemptsPerSide; i++)
            {
                ...
                // Overlay: squad role bias (spread or stack) for realism
                if (squad != null && squad.MembersCount > 1)
                {
                    candidate += UnityEngine.Random.insideUnitSphere * groupJitter;
                    if (i == MaxAttemptsPerSide - 1 && UnityEngine.Random.value < stackPenalty)
                        candidate = origin + (perpendicular * (BaseOffset * 0.53f)); // "gives up" and tries a more stacked position
                }
