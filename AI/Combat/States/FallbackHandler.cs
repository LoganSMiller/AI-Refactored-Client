// <auto-generated>
//   AI-Refactored: FallbackHandler.cs (Ultra-Platinum++, Max Realism Overlay Edition, June 2025)
...
namespace AIRefactored.AI.Combat.States
{
    ...
    /// <summary>
    /// Handles all overlay/intent-based fallback: NavMesh/door/personality/squad validated, event-only.
    /// Never disables, teleports, or assigns position. No vanilla fallback. Fully pooled, robust, and multiplayer/headless safe.
    /// Implements Ultra-Platinum++ layered intent, anticipation, squad negotiation, and fallback.
    /// </summary>
    public sealed class FallbackHandler : IDisposable
    {
        ...
        /// <summary>
        /// Overlay/intent-based: issues new fallback target, squad/personality/randomized offset, full NavMesh validation.
        /// </summary>
        ...
        /// <summary>
        /// Overlay/intent-only: issues a fallback path (never tick-moves or disables).
        /// </summary>
        ...
        /// <summary>
        /// Main fallback overlay logic—called by BotBrain. Only issues move if anticipated and safe; never disables, never tick-moves.
        /// Implements anticipation window, squad panic spread, retry/backup logic, and pooled, deduped move overlays.
        /// </summary>
        public void Tick(float deltaTime, float now, Action<CombatState, float> forceExit)
        {
            ...
            // 1. Squad panic spread (one time, AAA human layer)
            ...
            // 2. NavMesh validation & retry, with backup fallback on failure
            ...
            // Rate-limit actual move overlays (never per-frame, never spam, always dedup)
        ...
        private Vector3 FindBackupFallback(Vector3 origin)
        {
            ...
            Vector3 candidate = origin + UnityEngine.Random.insideUnitSphere * 2.7f;
            candidate.y = origin.y;

            if (TrySampleNavMesh(candidate, NavSampleRadius, out var navSafe))
                return ClampY(navSafe, origin);

            return ClampY(origin + Vector3.forward * 0.23f, origin);
        }
