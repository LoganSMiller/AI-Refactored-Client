// <auto-generated>
//   AI-Refactored: InvestigateHandler.cs (Supreme Arbitration Overlay/Event, Ultra-Realism+ June 2025)
...
namespace AIRefactored.AI.Combat.States
{
    ...
    /// <summary>
    /// Handles event-driven investigation overlays: NavMesh, squad, personality, anti-clump overlays.
    ...
    public sealed class InvestigateHandler
    {
        ...
        private const float PersonalityDelayVariance = 0.08f;
        ...
        private float _investigateStress = 0f;
        ...
        public Vector3 GetInvestigateTarget(Vector3 visualLastKnown)
        {
            ...
            // 1. Visual last known, 2. Own memory, 3. Squad memory, 4. Random probe
            ...
            // Squad/role/personality overlay
            result = ApplySquadOverlay(_bot, _cache, result);
            result = ApplyAntiClump(_bot, result);

            // Subtle anticipation/hesitation
            if (_investigateStress > 0.4f)
                result += UnityEngine.Random.insideUnitSphere * (MicroJitter * _investigateStress);
        ...
        public void Investigate(Vector3 target)
        {
            ...
            float now = Time.time;
            float delayBase = UnityEngine.Random.Range(MinHumanDelay, MaxHumanDelay);
            float caution = _cache.PersonalityProfile?.Caution ?? 0.1f;
            float anticipation = delayBase + caution * PersonalityDelayVariance + _investigateStress * 0.1f;

            if (now - _lastInvestigateTime < anticipation)
                return;

            // Arbitration: only one investigation overlay per bot per tick/type
            ...
            if (UnityEngine.Random.value < 0.24f)
                dest += UnityEngine.Random.insideUnitSphere * MicroJitter * (1f + _investigateStress);
            ...
            // Arrival/giveup/voice/stance logic
            if (!_hasArrived && distance < 1.1f)
            {
                _hasArrived = true;
                _investigateStress = Mathf.Clamp01(_investigateStress + 0.21f);

                float giveUpChance = GiveUpChanceBase + (caution * 0.6f) + (_investigateStress * 0.17f);
                if (!_hasGivenUp && UnityEngine.Random.value < giveUpChance)
                {
                    _hasGivenUp = true;
            ...
            {
                _hasArrived = false;
                _investigateStress = Mathf.Max(0f, _investigateStress - 0.16f); // stress drains if not stuck
            }

            if (_cache?.PoseController != null)
            {
                if (distance < 2.4f && UnityEngine.Random.value < 0.43f + caution * 0.25f)
        ...
        {
            // Personality, sound recency, no disables, and not given up
