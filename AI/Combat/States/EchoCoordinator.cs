// <auto-generated>
//   AI-Refactored: EchoCoordinator.cs (Supreme Arbitration Overlay/Event Edition, Ultimate Beyond Diamond – June 2025, Max Realism)
...
//   All overlay moves via BotOverlayManager/BotOverlayType, NavMesh+Y validation, post-NavMesh drift, dedup/cooldown, anticipation/lockout.
//   Full squad/role/personality overlays, squad leader/role logic, voice, and error isolation. MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Combat.States
{
    ...
    /// <summary>
    /// Overlay-only, squad-aware fallback, echo, and investigation overlays with supreme arbitration, anti-parallel, anti-clump, pooled, and error-shielded logic.
    /// All overlays are arbitration/event, NavMesh/Y safe, squad leader/personality aware, and SPT/FIKA/client/headless parity.
    /// </summary>
    public sealed class EchoCoordinator
    {
        ...
        private const float SquadLeaderGoChance = 0.78f;
        ...
        private const float VoiceRejoinChance = 0.18f;
        private const float IsolatedRejoinRadiusSqr = 529f;
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="EchoCoordinator"/>.
        /// </summary>
        ...
        /// <summary>
        /// Overlay fallback: every squadmate gets a triple-guarded, pooled, NavMesh/Y/drifted fallback overlay (never disables, anti-clump, pooled, leader/role/personality-aware).
        /// </summary>
        public void EchoFallbackToSquad(Vector3 retreatPosition)
        {
            try
            {
                ...
                bool isLeader = group.Member(leaderIdx) == _bot;
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    // Role/personality-aware fallback
                    bool leaderMove = (i == leaderIdx) || UnityEngine.Random.value > 0.19f + (mateCache?.PersonalityProfile?.Caution ?? 0.07f);
                    ...
                    if (leaderMove)
                    {
                        ...
                        // Cover/stance overlays and voice (randomized, role aware)
                        ...
                        // If rejoining from isolation, vocalize
                        if ((mate.Position - selfPos).sqrMagnitude > IsolatedRejoinRadiusSqr && UnityEngine.Random.value < VoiceRejoinChance)
                        {
        ...
        /// <summary>
        /// Squad overlay investigation: overlays/tactical comms, role/personality voice, never disables, pooled, anti-spam.
        /// </summary>
        public void EchoInvestigateToSquad()
        {
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    // Role/personality-based voice overlays (simulate nervous/confident/leader)
                    if (mate.BotTalk != null)
                    {
                        float voiceChance = VoiceInvestigateChance;
                        if (mateCache?.PersonalityProfile?.IsFearful == true)
                            voiceChance -= 0.15f;
                        if (mateCache?.PersonalityProfile?.AggressionLevel > 0.8f)
                            voiceChance += 0.13f;
                        if (UnityEngine.Random.value < voiceChance)
                        {
        ...
        /// <summary>
        /// Echoes enemy sighting event to squad: overlays comms, tactical memory, anti-clump, role/personality overlays, pooled.
        /// </summary>
        ...
        /// <summary>
        /// Applies post-NavMesh squad anti-clump/anti-parallel drift, leader/squad-aware, for max realism.
        /// </summary>
