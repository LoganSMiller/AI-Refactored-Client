// <auto-generated>
//   AI-Refactored: CombatStateMachine.cs (Beyond Diamond, BotMovementHelper-Only Edition, June 2025, Hitch/Teleport Fix Pass)
...
namespace AIRefactored.AI.Combat
{
    ...
    /// Overlay movement intent deduped/cooldowned, never double-issued/teleported.
    /// </summary>
    public sealed class CombatStateMachine
    {
        ...
        // Overlay-move dedup cache for fallback/engage/investigate/patrol
        ...
        public void Initialize(BotComponentCache componentCache)
        {
            ...
            _lastOverlayMoveIssued = Vector3.zero;
        ...
        public void Tick(float now)
        {
            ...
            if (_investigate?.ShallUseNow(now, _lastStateChangeTime) == true)
            {
                ...
                {
                    if (ShouldIssueOverlayMove(target))
                    {
                        _investigate.Investigate(target);
                        _lastOverlayMoveIssued = target;
                        _lastOverlayMoveTime = Time.time;
                        _lastStateChangeTime = now;
                    }
                }
                return;
            ...
            if (_patrol != null)
            {
                Vector3 patrolTarget = _bot.Position; // Would require exposing the planned target from PatrolHandler for dedup
                if (ShouldIssueOverlayMove(patrolTarget)) // For now, allow tick
                {
                    _patrol.Tick(now);
                    _lastOverlayMoveIssued = patrolTarget;
                    _lastOverlayMoveTime = Time.time;
                }
            }
        }
        ...
        /// Dedup/cooldown and paused/anticipation logic included per overlay-move fix plan.
        /// </summary>
        private void AssignFallbackIfNeeded(float now)
        {
            ...
            // Overlay dedup/cooldown/paused/anticipation guard
            if (ShouldIssueOverlayMove(fallback))
            {
        ...
        /// Should be called before issuing overlay/event-driven movement of any kind.
        /// </summary>
        private bool ShouldIssueOverlayMove(Vector3 intended)
        {
            // NOTE: Replace anticipation/fakeout checks with your actual overlay state checks if you have anticipation logic.
            if (BotMovementHelper.IsMovementPaused(_bot)) return false;
            if (BotMovementHelper.IsInInteractionState(_bot)) return false;
            if ((_lastOverlayMoveIssued - intended).sqrMagnitude < OverlayMoveDedupSqr) return false;
            if ((Time.time - _lastOverlayMoveTime) < OverlayMoveCooldown) return false;
            // Place anticipation/fakeout logic here if available.
            return true;
        }
