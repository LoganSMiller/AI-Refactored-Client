// <auto-generated>
//   AI-Refactored: CombatStateMachine.cs (Beyond Diamond++ Overlay Arbitration, Zero-Spam, June 2025, Hitch/Teleport/Skate Fix)
...
namespace AIRefactored.AI.Combat
{
    ...
    public sealed class CombatStateMachine
    {
        ...
        public void NotifyDamaged()
        {
            ...
            float now = Time.time;
            // Only trigger fallback if needed and not already active
        ...
        public void Tick(float now)
        {
            ...
            {
                if (_currentOverlayState != OverlayState.Fallback || !IsSameMove(_fallback.GetFallbackPosition()))
                {
                    OverlayMoveProposal(_fallback.GetFallbackPosition(), OverlayState.Fallback, now);
            ...
            if (EFTPlayerUtil.IsValid(enemy))
            {
                ...
                if (_engage?.ShallUseNow() == true)
                {
                    ...
                    {
                        OverlayMoveProposal(_lastKnownEnemyPos, OverlayState.Engage, now);
            ...
            if (_investigate?.ShallUseNow(now, _lastStateChangeTime) == true)
            {
                ...
                {
                    OverlayMoveProposal(investigateTarget, OverlayState.Investigate, now);
            ...
            if (_patrol != null)
            {
                ...
                {
                    OverlayMoveProposal(patrolTarget, OverlayState.Patrol, now);
                    _patrol.Tick(now);
                }
            }
        }
        ...
        private void OverlayMoveProposal(Vector3 proposed, OverlayState newState, float now)
        {
            if (!IsVectorValid(proposed)) return;
            ...
            // ONLY here: actually issue overlay move!
            BotMovementHelper.SmoothMoveToSafe(_bot, proposed, slow: true, _cache?.PersonalityProfile?.Cohesion ?? 1f);
        }
