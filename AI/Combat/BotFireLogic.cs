// <auto-generated>
//   AI-Refactored: BotFireLogic.cs (Supreme Arbitration Overlay/Event, Max Realism, June 2025)
//   All fire/aim/engagement overlays are one-shot, arbitration-guarded, humanized, pooled, and squad/cover aware.
//   No tick/coroutine-move, no per-frame spam, zero allocs, SPT/FIKA/client/headless parity, bulletproof error handling.
//   MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Combat
{
    ...
    /// <summary>
    /// Controls all weapon fire, aim, recoil, and cover overlays for AI bots.
    /// Integrates supreme arbitration: overlay-only, intent-driven, squad-aware, pooled, null-guarded, and fully event-locked.
    /// All failures are locally isolated. Full SPT/FIKA/headless/client safety and parity.
    /// </summary>
    public sealed class BotFireLogic
    {
        ...
        /// <summary>
        /// Overlay/event-driven fire and aim update. Only called by BotBrain.
        /// Supreme Arbitration: never tick/coroutine-moves, all moves/overlays arbitration-guarded, one-shot, bulletproof.
        /// </summary>
        public void Tick(float time)
        {
            ...
            try
            {
                ...
                // Overlay Arbitration: Only one overlay event-move per bot per intent
                    ...
                    if (BotOverlayManager.CanIssueMove(_bot, BotOverlayType.Attack))
                    {
                        if (profile.ChaosFactor > 0f && UnityEngine.Random.value < profile.ChaosFactor * 0.7f)
                        {
                            if (BotNavHelper.TryGetSafeTarget(_bot, out var advance) && IsVectorValid(advance))
                            {
                                ...
                                {
                                    BotMovementHelper.SmoothMoveToSafe(_bot, advance, slow: false, cohesion: 1f, overlayType: BotOverlayType.Attack);
                ...
                if (weaponManager.IsWeaponReady)
                {
                    ...
                    if (UnityEngine.Random.value < fireChance && !HasFriendlyInLineOfFire(aimPosition, profile))
                    {
