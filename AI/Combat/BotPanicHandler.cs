// <auto-generated>
//   AI-Refactored: BotPanicHandler.cs (Beyond Diamond – Hitch/Teleport Fix Pass, June 2025)
//   Ultra-realistic, squad-synced panic, stutter, suppression, fallback, and composure logic.
//   All movement overlays deduped, NavMesh/Y validated, anti-teleport, anti-loop, never spammed, never disables.
//   SPT/FIKA/headless/client parity. Zero alloc in hot path. StyleCop/ReSharper compliant. MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Combat
{
    ...
    /// <summary>
    /// Handles suppression, flash, injury, stutter, squad emotional contagion, retreat, composure decay, and squad calm sync.
    /// Overlay/event-only: all moves strictly deduped, intent-validated, pooled, NavMesh-safe, and zero-teleport.
    /// Bulletproof: All errors locally isolated. SPT, FIKA, headless/client parity.
    /// </summary>
    public sealed class BotPanicHandler
    {
        ...
        public void Tick(float time)
        {
            ...
            try
            {
                // Light probabilistic scan (spread tick load, anti-spam)
        ...
        private void TryStartPanic(float now, Vector3 retreatDir, bool isSquadSync)
        {
            ...
            try
            {
                // Overlay/intent-only: never issue panic move if anticipation, stutter, or interaction is pending.
                ...
                // Compose fallback (intent), NavMesh/Y-validated, with no direct assignment.
                Vector3 fallback = _bot.Position + retreatDir.normalized * UnityEngine.Random.Range(7.5f, 10.5f);
                fallback.y = _bot.Position.y;

                // Try use overlay intent, else NavHelper for safety (event/overlay-only, never raw)
                if (BotNavHelper.TryGetSafeTarget(_bot, out var navTarget) && IsVectorValid(navTarget))
                    fallback = navTarget;

                // Dedup/cooldown strictly on the final NavMesh-sampled, drifted overlay position.
                    ...
                    now - _lastOverlayMoveTime > OverlayMoveCooldown)
                {
                    ...
                    // **No direct assignment: all movement overlays go through bulletproof helper**
                ...
                // Never update cache if move isn't issued
            }
            catch (Exception ex)
            {
