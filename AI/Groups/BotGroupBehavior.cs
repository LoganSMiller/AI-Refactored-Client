// <auto-generated>
//   AI-Refactored: BotGroupBehavior.cs (Beyond Diamond Overlay/Event Edition – June 2025)
//   Ultimate realism: flocking, dynamic roles, leadership, emotion, squad comms, event/overlay movement only.
//   All movement is one-shot, pooled, NavMesh safe, cooldown/position deduped, never tick-spammed.
//   MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Groups
{
    ...
    /// <summary>
    /// Drives ultra-realistic squad flocking, role assignment, comms, and overlays.
    /// All logic is pooled, NavMesh/spacing-safe, zero alloc, event-driven only.
    /// </summary>
    public sealed class BotGroupBehavior
    {
        ...
        public void Tick(float deltaTime)
        {
            ...
            {
                TryIssueGroupMove(SmoothDriftMove(myPos, repulsion.normalized * RepulseStrength, deltaTime, true), now);
            ...
            if (hasFurthest)
            {
                ...
                {
                    TryIssueGroupMove(SmoothDriftMove(myPos, dir.normalized * MaxSpacing * 0.69f, deltaTime, false), now);
                    IsFollowingLeader = false;
                    return;
                }
            }
            ...
            if (closestIdx >= 0 && closestDist > 0.65f && closestDist < MaxSquadRadiusSqr)
            {
                ...
                {
                    TryIssueGroupMove(SmoothDriftMove(myPos, leaderDir.normalized * (MinSpacing * 0.8f), deltaTime, false), now);
        ...
        private void TryIssueGroupMove(Vector3 target, float now)
        {
            ...
            if (_moveCache.IsNewTarget(target, SpacingTolerance * SpacingTolerance) &&
                (now - _moveCache.LastMoveTime) > MinTickMoveInterval)
            {
                if (!NavMesh.SamplePosition(target, out NavMeshHit navHit, 1.4f, NavMesh.AllAreas))
                    return;
