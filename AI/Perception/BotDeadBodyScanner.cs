// <auto-generated>
//   AI-Refactored: BotDeadBodyScanner.cs (Ultimate Overlay-Only, Pooled/Null-Safe, June 2025)
//   SYSTEMATICALLY MANAGED. Movement/event overlays only. Squad-safe, pooled, multiplayer/headless compliant, never disables.
//   MIT License.
// </auto-generated>
...
namespace AIRefactored.AI.Looting
{
    ...
    /// <summary>
    /// Handles bot logic for intent-based corpse looting—never ticked, never disables. Only event/overlay.
    /// </summary>
    public sealed class BotDeadBodyScanner
    {
        ...
        private void TryLootOnce()
        {
            ...
            try
            {
                ...
                // Intent: Move to corpse only if not already in range (no tick-move, no spam)
                Vector3 corpsePos = corpse.Transform.position;
                float dist = Vector3.Distance(_bot.Position, corpsePos);
                if (dist > MoveToCorpseRadius)
                {
                    // Only issue real move if not recently moved (cache/cooldown checked)
                    if (_cache.MoveCache != null && _cache.MoveCache.CanIssueMove(corpsePos, Time.time))
                    {
                        ...
                        _cache.MoveCache.LastIssuedTarget = corpsePos;
                        _cache.MoveCache.LastMoveTime = Time.time;
                    }
                    return;
                }

                // Now in range—loot it
                LootCorpse(corpse);
