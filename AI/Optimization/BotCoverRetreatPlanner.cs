// <auto-generated>
//   AI-Refactored: BotCoverRetreatPlanner.cs (Ultimate Human-Like Retreat, Cover, and Squad Flow – June 2025 Absolute Max-Realism Edition)
...
namespace AIRefactored.AI.Optimization
{
    ...
    /// <summary>
    /// Supreme squad/chaos/personality-driven cover/fallback/retreat planner for AIRefactored bots.
    /// No direct path or move. Overlay/event-only arbitration. All logic pooled, SPT/FIKA/headless/client parity,
    /// anti-jam, anti-flicker, bulletproof. Expansion-ready for further tactical overlays.
    /// </summary>
    public sealed class BotCoverRetreatPlanner
    {
        ...
        private const float ChaosOffsetRadius = 2.8f;
        private const float CoverProbeRadius = 4.3f;
        private const int CoverProbeRays = 16;
        private const float CoverCheckInterval = 0.21f;
        private const float CoverPersistTime = 0.75f;
        private const float NavSampleRadius = 1.6f;
        private const float MinRetreatDist = 7.7f;
        private const float MinCoverDot = 0.55f;
        private const float MinPanicContagionChance = 0.19f;
        private const float MaxGroupSizeToSync = 6;

        #endregion

        #region Static Pool and Cache

        // Per-map, per-squad retreat intent overlays.
        ...
        public List<Vector3> GetCoverRetreatPath(Vector3 threatDir)
        {
            ...
            try
            {
                ...
                if (overlayPath != null && overlayPath.Count >= 2)
                {
                    ...
                    // Squad overlay: propagate panic/fakeout overlays for realism
                    SquadPanicContagion(_bot, overlayPath);
        ...
        public bool TryGetBestCoverNear(Vector3 target, Vector3 current, out Vector3 coverPoint)
        {
            ...
            if (now - _lastCoverCheckTime < CoverCheckInterval)
            {
                if (_isInCover)
                {
                    coverPoint = NavMeshSampleSafe(current + _coverNormal * 1.18f, current);
                    return true;
                }
                return false;
            }
            ...
            for (int i = 0; i < CoverProbeRays; i++)
            {
                ...
                if (!NavMesh.SamplePosition(probe, out NavMeshHit navHit, 2.2f, NavMesh.AllAreas))
                    continue;
                ...
                float score = dot * 2.1f - Vector3.Distance(current, navPos) * 0.17f;
                Vector3 eye = navPos + Vector3.up * 1.38f;
            ...
            if (foundCover && bestScore > float.MinValue + 0.31f)
            {
                _isInCover = true;
        ...
        private static bool SquadOverlapCheck(BotOwner bot, List<Vector3> path)
        {
            ...
            return Vector3.Distance(bot.Position, last) < 2.3f;
        }
        ...
        private static void ApplyChaosOffset(List<Vector3> path, BotOwner bot, Vector3 threatDir)
        {
            ...
            if (cache.Suppression != null && cache.Suppression.IsSuppressed())
                chaosMul += 0.45f;
            chaosMul += (1f - Mathf.Clamp01(profile.Caution)) * 0.29f;

            for (int i = 1; i < path.Count; i++)
            {
                float chaos = ChaosOffsetRadius * chaosMul * UnityEngine.Random.Range(0.75f, 1.19f);
        ...
        /// <summary>
        /// Squad panic/fakeout overlay propagation for cover/fallback realism.
        /// </summary>
        private static void SquadPanicContagion(BotOwner bot, List<Vector3> path)
        {
            try
            {
                if (bot == null || bot.BotsGroup == null || path == null || path.Count < 2)
                    return;

                int squadCount = Math.Min(bot.BotsGroup.MembersCount, (int)MaxGroupSizeToSync);
                var cache = bot.GetComponent<BotComponentCache>();
                if (cache?.PanicHandler == null || !cache.PanicHandler.IsPanicking)
                    return;
                ...
                for (int i = 0; i < squadCount; i++)
                {
                    ...
                    if (mateCache?.PanicHandler != null && mateCache.PanicHandler.GetComposureLevel() > 0.5f &&
                        UnityEngine.Random.value < MinPanicContagionChance && mateCache.GroupComms != null)
                    {
            ...
            {
                Plugin.LoggerInstance.LogWarning("[BotCoverRetreatPlanner] SquadPanicContagion error: " + ex);
            }
        }
